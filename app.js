...
const LOGO_HOLDING = "logo-holding.png";
const LOGO_SAFETY  = "logo-safety.png";
...
function toggleStatus(btn, fieldId) {
    ...
    document.getElementById(fieldId).value =
        btn.classList.contains('compliant') ? 'مطابق' : 'غير مطابق';
}
...
function toggleStatusCrane(btn, fieldId) {
    ...
    btn.classList.add('active');
    document.getElementById(fieldId).value =
        btn.classList.contains('exist') ? 'يوجد' : 'لا يوجد';
}
...
function getGPSLocation() {
    ...
    if (!navigator.geolocation) {
        ...
        coordsDiv.classList.add('visible'); return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        function(position) {
            ...
            coordsDiv.textContent = '⏳ جاري تحديد اسم الموقع...';
            ...
            fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat=' + lat + '&lon=' + lng + '&accept-language=ar')
                ...
                .then(function(data) {
                    ...
                    if (data.address) {
                        ...
                        var unique = parts.filter(function(v,i,a){ return a.indexOf(v)===i; });
                        locationName = unique.join(' - ');
                    }
                    if (!locationName && data.display_name) locationName = data.display_name;
                    if (!locationName) locationName = lat + ', ' + lng;

                    coordsDiv.innerHTML =
                        '<div style="margin-bottom:6px;"><i class="fas fa-map-marker-alt"></i> <strong>الموقع:</strong> ' + locationName + '</div>' +
                        '<div style="font-size:11px;opacity:0.8;"><i class="fas fa-crosshairs"></i> الإحداثيات: ' + lat + ', ' + lng + '</div>' +
                        '<a href="https://maps.google.com/?q=' + lat + ',' + lng + '" target="_blank" style="color:var(--primary);font-weight:bold;display:inline-block;margin-top:4px;">📍 فتح في خرائط جوجل</a>';
                ...
                .catch(function() {
                    coordsDiv.innerHTML =
                        ...
                        '<a href="https://maps.google.com/?q=' + lat + ',' + lng + '" target="_blank" style="color:var(--primary);font-weight:bold;">📍 فتح في خرائط جوجل</a>';
                    if (!locationInput.value.trim()) locationInput.value = lat + ', ' + lng;
                });
        },
        function(error) {
            var msg = '❌ تعذر تحديد الموقع. ';
            if (error.code === 1) msg += 'فعّل صلاحية الموقع';
...
function handlePhotos(input) {
    ...
    if (currentPhotos.length + files.length > 12) {
        showToast('الحد الأقصى 12 صورة فقط', true); return;
    }
...
function saveReport() {
    ...
    var report = {
        ...
        gps: {
            lat: coordsDiv.dataset.lat || '',
            lng: coordsDiv.dataset.lng || ''
        },
        designCapacity:     design ? parseFloat(design) : null,
        actualCapacity:     actual ? parseFloat(actual) : null,
        capacityUnit:       unit,
        capacityPercentage: (design && actual)
            ? Math.round((parseFloat(actual)/parseFloat(design))*100) : null,
...
function saveAndExportPDF() {
    ...
    if (report) setTimeout(function(){ generatePDF(report); }, 500);
}
...
function generatePDF(report) {
    ...
    if (report.designCapacity && report.actualCapacity) {
        ...
        capacityHTML =
            ...
            '<tr><td>نسبة التشغيل</td><td>' +
            '<div class="pdf-capacity-bar"><div class="pdf-capacity-fill" style="width:' + Math.min(pct,100) + '%;background:' + barColor + ';">' + pct + '%</div></div>' +
            '</td></tr>';
    }

    var isoHTML = report.isoStatus === 'حاصلة'
        ? '✅ حاصلة' +
          (report.isoType ? ' - النوع: ' + report.isoType : '') +
          (report.isoYear ? ' - سنة الحصول: ' + report.isoYear : '')
        : '❌ غير حاصلة';

    var tsmHTML = report.tsmStatus === 'حاصلة'
        ? '✅ حاصلة' + (report.tsmYear ? ' - سنة الحصول: ' + report.tsmYear : '')
        : '❌ غير حاصلة';
    ...
    if (report.photos && report.photos.length > 0) {
        var photosGrid = report.photos.map(function(p, i) {
            return '<div style="width:24%;margin:0.5%;position:relative;">' +
                ...
                '<div style="position:absolute;bottom:2px;right:2px;background:rgba(0,0,0,0.6);color:#fff;padding:1px 5px;border-radius:3px;font-size:8px;font-weight:bold;">' + (i+1) + '</div>' +
                '</div>';
        }).join('');
        photosHTML =
            '<div class="pdf-section">' +
            '<h3>📷 صور الملاحظات (' + report.photos.length + ' صورة)</h3>' +
            '<div style="display:flex;flex-wrap:wrap;gap:0;">' + photosGrid + '</div>' +
            '</div>';
    }
    ...
    var template =
        ...
        '<div class="pdf-header-logos">' +
        '<img src="' + LOGO_HOLDING + '" alt="شعار الشركة القابضة" style="width:70px;height:70px;object-fit:contain;border-radius:8px;background:#fff;padding:4px;border:2px solid rgba(255,255,255,.3);display:block;">' +
        '<div class="pdf-header-titles">' +
        '<h1>الشركة القابضة لمياه الشرب والصرف الصحي</h1>' +
        '<h2>الإدارة العامة للسلامة والصحة المهنية والطوارئ والأزمات</h2>' +
        '<h3>🚰 تقرير المرور على محطات مياه الشرب والصرف الصحي</h3>' +
        '</div>' +
        '<img src="' + LOGO_SAFETY + '" alt="شعار الإدارة العامة" style="width:70px;height:70px;object-fit:contain;border-radius:8px;background:#fff;padding:4px;border:2px solid rgba(255,255,255,.3);display:block;">' +
        '</div>' +
        '<div class="pdf-meta">📅 ' + dateFormatted + ' &nbsp;|&nbsp; 🏭 ' + report.stationType + ' - ' + report.stationName + '</div>' +
        '</div>' +

        '<div class="pdf-section"><h3>📋 المعلومات الأساسية</h3>' +
        ...
        capacityHTML +
        '<tr><td>مسؤول السلامة والصحة المهنية</td><td>' + report.safetyOfficer + '</td></tr>' +
        '<tr><td>مدير المحطة</td><td>' + report.stationManager + '</td></tr>' +
        '<tr><td>شهادة الايزو (ISO)</td><td>' + isoHTML + '</td></tr>' +
        '<tr><td>شهادة TSM</td><td>' + tsmHTML + '</td></tr>' +
        '</table></div>' +

        '<div class="pdf-section"><h3>🔍 بنود الفحص</h3>' +
        ...
        '<div class="pdf-section"><h3>📊 التقييم العام</h3>' +
        '<table class="pdf-table">' +
        '<tr><td>الحالة العامة</td><td><span class="pdf-status-badge ' + statusClass + '">' + report.overallStatus + '</span></td></tr>' +
        '<tr><td>درجة الخطورة</td><td>' + report.severity + '</td></tr>' +
        '</table></div>' +

        (report.notes ? '<div class="pdf-section"><h3>📝 الملاحظات العامة</h3><p style="font-size:13px;line-height:1.8;">' + report.notes + '</p></div>' : '') +
        ...
        '<p>الشركة القابضة لمياه الشرب والصرف الصحي</p>' +
...
function renderReportsList() {
    ...
    if (reports.length === 0) {
        container.innerHTML =
            ...
            '<p style="font-size:13px;">اضغط على "تقرير جديد" لإضافة أول تقرير</p>' +
            '</div>';
        return;
    }
    ...
    container.innerHTML = reports.map(function(report) {
        ...
        return '<div class="report-card ' + statusClass + '" onclick="viewReport(' + report.id + ')">' +
            ...
            '<p><i class="fas fa-user-tie"></i> ' + report.inspectorName + '</p>' +
            '</div>' +
...
function filterReports() {
    ...
    reports.forEach(function(report, index) {
        ...
        if (cards[index])
            cards[index].style.display = (matchSearch && matchStatus) ? 'flex' : 'none';
    });
}
...
function viewReport(id) {
    ...
    var isoCertHTML =
        '<div class="detail-row"><span class="detail-label">شهادة الايزو (ISO)</span><span class="detail-value">' +
    ...
    if (report.photos && report.photos.length > 0) {
        photosHTML =
            '<div class="detail-section"><h3><i class="fas fa-camera"></i> الصور</h3>' +
    ...
    document.getElementById('reportDetail').innerHTML =
        ...
        '<div class="detail-row"><span class="detail-label">درجة الخطورة</span><span class="detail-value">' + report.severity + '</span></div>' +
        '</div>' +

        (report.notes ? '<div class="detail-section"><h3><i class="fas fa-sticky-note"></i> الملاحظات العامة</h3><p style="line-height:1.8;font-size:14px;">' + report.notes + '</p></div>' : '') +
        ...
        '<button class="btn btn-danger" style="flex:1;" onclick="deleteReport(' + report.id + ');showScreen(\'reportsList\');"><i class="fas fa-trash"></i> حذف التقرير</button>' +
        '</div>';
