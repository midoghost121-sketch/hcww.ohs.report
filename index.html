...
function handlePhotos(inp){var fs=Array.from(inp.files);if(currentPhotos.length+fs.length>12){showToast('الحد 12 صورة',true);return;}fs.forEach(function(f){var r=new FileReader();r.onload=function(e){currentPhotos.push({name:f.name,data:e.target.result});renderPhotoPreview();};r.readAsDataURL(f);});inp.value='';}
...
function saveReport(){
    ...
    showToast('تم حفظ التقرير ✅');return report;
}
...
function generatePDF(report){
    ...
    var t='<div class="pdf-content"><div class="pdf-header"><div class="pdf-header-logos">'+
        ...
        '</div><div class="pdf-meta">📅 '+df+' | 🏢 '+report.companyName+' | 🏭 '+report.stationName+' - '+report.branch+'</div></div>'+
        ...
        '<div class="pdf-section"><h3>📊 التقييم العام</h3><table class="pdf-table"><tr><td>الحالة</td><td><span class="pdf-status-badge '+sc+'">'+report.overallStatus+'</span></td></tr><tr><td>الخطورة</td><td>'+report.severity+'</td></tr></table></div>'+
    ...
    html2pdf().set({margin:[10,10,15,10],filename:fileName,image:{type:'jpeg',quality:0.95},html2canvas:{scale:2,useCORS:true,scrollY:0},jsPDF:{unit:'mm',format:'a4',orientation:'portrait'},pagebreak:{mode:['avoid-all','css','legacy']}})
        .from(td).outputPdf('blob').then(function(pdfBlob){
            td.style.display='none';
            var link=document.createElement('a');link.href=URL.createObjectURL(pdfBlob);link.download=fileName;link.click();
            uploadPDF(pdfBlob,fileName).then(function(){showScreen('mainScreen');});
        }).catch(function(){td.style.display='none';showToast('خطأ PDF',true);});
}
